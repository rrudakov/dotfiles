...
(use-package recentf
  :custom
  (recentf-save-file (expand-file-name "recentf" rr-savefile-dir))
    ...
    (let ((bg "#3F3F3F")
      ...
      (with-selected-frame frame
        ...
        (custom-theme-set-faces
         ...
         `(git-gutter-fr:added ((t (:background ,bg))))
         `(git-gutter-fr:deleted ((t (:background ,bg))))
         `(git-gutter-fr:modified ((t (:background ,bg))))
...
(use-package minions
  :custom
  (minions-direct '(pyvenv-mode projectile-mode))
  :config
  (minions-mode 1))
#+end_src
** Shell settings
...
(use-package mu4e
  ...
  :custom
  (mu4e-maildir (expand-file-name "~/.mail/ProtonMail"))
...
(use-package mu4e-alert
  :custom
  (mu4e-alert-modeline-formatter (lambda (count)
                                   (if (> count 0)
...
(use-package lsp-java
  ...
  (setq lsp-java-vmargs '("-javaagent:/home/rrudakov/Soft/lombok/lombok.jar" "-Xmx1G" "-XX:+UseG1GC" "-XX:+UseStringDeduplication"))
...
**** Custom bindings
#+BEGIN_SRC emacs-lisp
(define-key python-mode-map (kbd "C-c j") #'sp-split-sexp)
#+END_SRC
*** JSON
#+BEGIN_SRC emacs-lisp
