    ...
    (when (buffer-file-name)
      (pcase (vc-state (buffer-file-name))
        (`up-to-date
         (concat
          (propertize ""
                      'face `(:inherit mode-line :height 1.2)
                      'display '(raise -0.1))
          (propertize (format " %s" branch) 'face `(:inherit mode-line))))
        (`edited
         (concat
          (propertize ""
                      'face `(:inherit mode-line :foreground "#87afaf" :height 1.2)
                      'display '(raise -0.1))
          (propertize (format " %s" branch) 'face `(:inherit mode-line :foreground "#87afaf"))))
        (`added
         (concat
          (propertize ""
                      'face `(:inherit mode-line :foreground "#b8bb26" :height 1.2)
                      'display '(raise -0.1))
        ...
        (`removed
         (concat
          (propertize ""
                      'face `(:inherit mode-line :foreground "#fb4934" :height 1.2)
                      'display '(raise -0.1))
...
(set-face-attribute 'spaceline-python-venv nil
                    :foreground "#b8bb26" :distant-foreground "#d3869b")
#+END_SRC
***** Moody
...
(use-package lsp-mode
  ...
  ;; (setq ms-pyls-dir (expand-file-name "~/Soft/Microsoft/python-language-server/output/bin/Release/"))
