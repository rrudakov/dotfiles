    ...
    (let ((dim "dim gray")
      ...
      (zenburn-with-color-variables
       (custom-theme-set-faces
        ...
        `(org-tag ((t (:background ,zenburn-red-5 :foreground ,zenburn-fg)))))))
...
(use-package dap-mode
  ...
  (setq dap-connect-retry-interval 0.5
  ...
  ;; TODO: select test module name
  (defun rr/debug-test ()
    (interactive)
    (let* ((port "1044")
           (method-name (dap-java-test-method-at-point))
           (current-workdir (projectile-project-root))
           (project-name (file-name-nondirectory (directory-file-name current-workdir)))
           (command (format "mvn clean install -U -DskipTests && mvn -Dit.test=%s verify -pl integration-tests -P tests-for-development -Dmaven.failsafe.debug=\"-Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=y,address=%s -Xnoagent\"" method-name port)))
      (dap-debug
       (list :type "java"
             :request "attach"
             :hostName "localhost"
