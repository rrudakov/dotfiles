...
if it's not installed yet. For build-in packages it's required to use ~:ensure
nil~ to prevent attempt to install it from MELPA.
...
*** Do not auto save messages in the minibuffer
...
I want to keep maximum 10 last versions of file to be able to restore it to
previous state. It's very rare case and I don't think I need more that 10
backup files. To keep emacs directory clean I create separate directory where
I'm going to store all backup and autosave files. Also I'm gonna store helm
history file and org clock file in this directory and maybe something else
related to restoring files and states.
...
For all modes derived from ~text-mode~ I'd like to enable ~auto-fill-mode~ to
restrict maximum line length to ~current-fill-column~ value. When line length
reach this value line break will be inserted automatically.
...
** Files local variables behavior
    ...
    (when (member 'modus-vivendi custom-enabled-themes)
      (modus-vivendi-theme-with-color-variables
        (custom-theme-set-faces
         ...
         `(org-done ((t (:foreground ,fg-alt :strike-through t))))))))
...
#+end_src
** Attempt to fix performance problem with long lines
So long mode:
#+begin_src emacs-lisp
(use-package so-long
  :config
...
;; Modus vivendi
(setq org-todo-keyword-faces
      '(("PROGRESS" . (:foreground "#29aeff"))
        ("WAITING" . (:foreground "#eecc00"))
        ("REVIEW" . (:foreground "#eecc00"))
        ("CANCELLED" . (:foreground "#a8a8a8"))))
#+END_SRC
