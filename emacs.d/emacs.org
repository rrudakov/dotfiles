...
Draw underscore line at very bottom.
...
** Replace selected text on input, aka delete-selection-mode
...
(use-package esh-mode
  ...
  (add-hook 'eshell-first-time-mode-hook #'rr/eshell-init)
...
** Accept advice functions redefinition quietly
  ...
  (defun rr/apply-theme ()
    ...
    (zenburn-with-color-variables
      (custom-theme-set-faces
       ...
       `(helm-selection ((t (:background ,zenburn-bg+1 :underline ,zenburn-fg-1 :distant-foreground nil))))
       ...
       ;; CIDER
       `(cider-deprecated-face ((t (:background ,zenburn-bg+2))))
       ...
       `(ahs-face ((t (:inherit highlight :weight bold))))
       `(ahs-plugin-defalt-face ((t (:inherit highlight :weight bold))))
       `(ahs-plugin-whole-buffer-face ((t (:inherit highlight :weight bold))))
       `(ahs-definition-face ((t (:inherit highlight :underline t))))
       ;; Some org-mode faces
       `(org-mode-line-clock ((t (:background nil))))
       `(org-headline-todo ((t (:foreground ,zenburn-red+2))))
       `(org-drawer ((t (:foreground ,zenburn-blue-2))))
       `(org-ellipsis ((t (:background ,zenburn-bg :foreground ,zenburn-yellow-1 :box nil))))
       `(org-tag ((t (:background ,zenburn-bg-05
                                  :foreground ,zenburn-fg-05
                                  :box (:line-width -1 :color ,zenburn-fg-05)))))
       ...
       ;; Markdown
       `(markdown-code-face ((t (:inherit fixed-pitch :background ,zenburn-bg+05 :extend t))))
       ;; Tree sitter
       `(tree-sitter-hl-face:function.call ((t (:inherit default))))
       `(tree-sitter-hl-face:attribute ((t (:foreground ,zenburn-blue+1))))
       `(tree-sitter-hl-face:operator ((t (:foreground ,zenburn-green+4))))
       `(tree-sitter-hl-face:constant ((t (:foreground ,zenburn-orange))))
       `(tree-sitter-hl-face:constructor ((t (:inherit font-lock-builtin-face))))
       `(tree-sitter-hl-face:label ((t (:foreground ,zenburn-green+4))))
       `(tree-sitter-hl-face:variable.parameter ((t (:foreground ,zenburn-cyan))))
       ...
       ;; Emacs 28
       `(help-key-binding ((t (:background ,zenburn-bg-1
...
(use-package solarized-theme
  ...
  (defun rr/apply-solarized-theme ()
    "Apply solarized theme."
    (load-theme 'solarized-dark t)
    (enable-theme 'solarized-dark)
    (remove-hook 'server-after-make-frame-hook #'rr/apply-solarized-theme))

  (if (daemonp)
      (add-hook 'server-after-make-frame-hook #'rr/apply-solarized-theme)
    (rr/apply-solarized-theme)))
#+end_src
...
*** COMMENT Tron legacy theme

#+begin_src emacs-lisp
(use-package tron-legacy-theme
  :config
  (defun rr/apply-tron-theme ()
    "Apply tron theme."
    (load-theme 'tron-legacy t)
    (enable-theme 'tron-legacy)
    (remove-hook 'server-after-make-frame-hook #'rr/apply-tron-theme))

  (if (daemonp)
      (add-hook 'server-after-make-frame-hook #'rr/apply-tron-theme)
    (rr/apply-tron-theme)))
#+end_src
...
(defun rr/set-font (height)
  ...
  (set-face-attribute 'default nil :family fixed-pitch-font-family :weight 'normal :height height)
  (set-face-attribute 'fixed-pitch nil :family fixed-pitch-font-family :weight 'normal :height height)
  (set-face-attribute 'variable-pitch nil :family variable-pitch-font-family :weight 'normal :height (floor (* height 1.1))))
...
(use-package tree-sitter-langs
  ...
  (tree-sitter-hl-add-patterns 'java
    [
     ...
     ;; class as argument
     ;; Example: SomeJava in `methodInvocation(SomeJava.class);'
     (method_invocation
      (argument_list
       (field_access
        (identifier)+ @type)))
     ...
     ;; Example: EnumClass in `EnumClass.SOME_VALUE.getValue()'
     (method_invocation
      (field_access
       (identifier)+ @type))

     ;; Example: State in `@EnumSource(value = State.class)'
     (annotation_argument_list
      (element_value_pair
       (identifier)
       (field_access
     ...
     ;; Example: SoftAssertionsExtension in `@ExtendWith(SoftAssertionsExtension.class)'
     (annotation_argument_list
      (field_access
       (identifier)+ @type))

     ;; Example: DependencyInjectionTestExecutionListener in
  ...
  ;; The following works but there are no queries yet for TSX syntax
...
(use-package org
  ...
  (setq org-adapt-indentation nil                                        ; Do not align content with headline level
        ...
        org-src-preserve-indentation t                                   ; Preserve indentation of source code blocks on export and on switch to edit block
        ...
        org-fontify-done-headline nil                                    ; Add special face for the entire done heading and not only keyword
        org-fontify-todo-headline nil                                    ; Add special face for the entire todo heading and not only keyword
...
(use-package org-mime
  ...
  (add-hook 'org-mime-html-hook
            (lambda ()
              (org-mime-change-element-style
               "blockquote" "border-left: 2px solid gray; padding-left: 2px;")))
...
(use-package dtrt-indent
  ...
  (add-hook 'c-mode-common-hook #'dtrt-indent-mode 99)
  (add-hook 'prog-mode-hook #'dtrt-indent-mode 99))
#+end_src
...
(use-package flycheck-posframe
  ...
  (with-eval-after-load 'flycheck
    (setq flycheck-posframe-borderc-width 1
          flycheck-posframe-position 'window-bottom-left-corner
          flycheck-posframe-inhibit-functions '(lambda () (bound-and-true-p company-backend)))
...
(use-package cider
  ...
  ;; (defun rr/cider-jump-to-definition (&optional arg)
  ;;   "My fallback to `xref-find-definitions' for CIDER."
  ;;   (interactive "P")
  ...
  ;;               (cider--find-var-other-window sym)
  ;;             (cider--find-var sym))
  ;;         (error (call-interactively #'xref-find-definitions))))))

  ;; (define-key cider-mode-map [remap cider-find-var] #'rr/cider-jump-to-definition)
  ...
  (setq cider-jdk-src-paths '("~/Soft/java/clojure-1.10.3-sources"
                              "~/Soft/java/openjdk-11-src"))
...
(use-package clj-refactor
  ...
  (add-to-list 'cljr-libspec-whitelist "^next.jdbc.date-time$")
...
** TODO Consider switching to [[https://github.com/tecosaur/emacs-everywhere][emacs-everywhere]]
...
[[https://github.com/ledger/ledger][Ledger]] is command line utility to track your finances. It works with plain text
files in certain format. The following major mode brings support for =.ledger=
files to emacs.
